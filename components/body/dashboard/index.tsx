...
import { getNextMatch, insertAttendedMember, checkIfUserRegisterCompete, removeAttendedMember, getAttendedMember, deleteAttendedMember } from '@/lib/api/match'
...
const index: React.FC = () => {
	...
	const [isRegisterCompeteLoading, setIsRegisterCompeteLoading] = useState<boolean>(false)
	...
	const changeTimeAndPlace = (data: any) => {
		...
		const formattedDate = `${hours} giờ ${minutes} phút. Ngày ${day < 10 ? '0' : ''}${day}-${month < 10 ? '0' : ''}${month}-${year}`;
		if (date >= today) {
			...
			{isLoading ?
				...
				<div className='flex flex-col gap-4 pb-8' >
					...
					<div className='bg-white shadow-md mt-2 rounded-lg p-4 flex flex-col gap-4'>
						...
						<p className='text-lg '>Thời gian: <span className='text-red-500 font-semibold'>{nextMatchTime}</span></p>
						<p className='text-lg '>Địa điểm: <span className='text-red-500 font-semibold'>{nextMatchPlace} {filedNumber}</span></p>
					...
					<div className='bg-white shadow-md mt-2 rounded-lg p-4'>
						<div>
							...
							>
								...
								<div className='mt-3'>
									{isRegisterCompete
										? <Tag color="green"><p className='text-lg'>Bạn đã đăng ký thi đấu</p></Tag>
										: <Tag color="volcano"><p className='text-lg'>Bạn chưa đăng ký thi đấu</p></Tag>}
								</div>
						...
						<div className='mt-4'>
							...
							<Table dataSource={dataRegisterCompeteTable} columns={columnsAttendedMember} loading={isRegisterCompeteLoading} />
						</div>
					</div>
					<div className='bg-white shadow-md mt-2 rounded-lg p-4'>
