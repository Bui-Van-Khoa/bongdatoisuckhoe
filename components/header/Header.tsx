      ...
      {isLoading ? (
        ...
        >
            ...
            <ul className="flex md:gap-8 gap-4">
              {Object.values(navMenu).map((item, index) => (
                <Link href={item.link} key={index}>
                  <li
                    className={`text-lg font-medium hover:text-blue-800 p-3 ${
                      pathName === item.link ? 'text-blue-800' : 'text-black'
                    }`}
                  >
                    {item.title}
                  </li>
          ...
          <div>
            {shouldRender && (
              <div
                className={`bg-white h-screen w-screen flex justify-between px-10 py-6 absolute top-0 z-10 ${
                  show ? 'mountedStyle' : 'unMountedStyle'
                }`}
