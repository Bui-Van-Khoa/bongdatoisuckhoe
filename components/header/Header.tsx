	...
	return (
		<>
			{isLoading
				? <LoadingComponent />
				:
				<HeaderStyled className={`relative ${isShowHeader ? 'hidden' : 'block'}`}>
					<div className='flex justify-between bg-amber-200 px-3 pt-3 sm:px-10 items-center' >
						<div className='flex items-center gap-4 '>
							<Image src={logo} width={80} height={80} />
							<p className='text-blue-900 text-3xl font-medium'>Bóng đá sức khỏe tối Football club</p>
						</div>
						...
						{shouldRender &&
							...
							>
								<div className=''>
									...
									<ul>
										{Object.values(navMenu).map((item, index) => (
											<li key={index} className='pb-4 text-lg font-medium'> {item.title}</li>
										))}
									</ul>
								</div>
								<div>
